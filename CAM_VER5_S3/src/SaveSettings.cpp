...
void NVSReadSettings() {
  ...
  bool ok = preferences.begin(NVS_NAMESPACE_CONFIG, true);  // read-only mode
  /*
  if (!ok) {
    log_n("NVS Config open failed!");
    MyConfig.wifiSsid       = DEFAULT_WIFI_SSID;
    MyConfig.wifiPass       = DEFAULT_WIFI_PASS;
    NVSWriteSettings();
    return;
  }
  */
